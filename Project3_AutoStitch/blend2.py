...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    for x in range(minY,maxY):
        for y in range(minX,maxX):
            ...
            # print "x, y",(img_x, img_y)
            # Skip if out of bounds
            if (img_x < 0 or img_x >= img_width or img_y < 0 \
                or img_y >= img_height):
                continue

            rgb_coord = np.zeros(3)
